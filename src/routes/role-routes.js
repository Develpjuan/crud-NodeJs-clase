...
router.post("/", async(req, res) => {
    ...
    try {
        await db.connect();
        const dataQry = [req.body.role_name];
        const qry = `INSERT INTO role(role_name) VALUES (?);`;
...
router.put("/:id", async(req, res) => {
    ...
    try {
        ...
        if(Object.keys(getModule).length != 0) {
            const dataQry = [req.body.role_name];
            const qry = `UPDATE role SET role_name = ? WHERE role_id = ${req.params.id};`;
            //Execute a query
            const results = await db.query(qry, dataQry);
...
router.delete("/:id", async(req, res) => {
    ...
    try {
        ...
        if(Object.keys(getRole).length != 0) {
            ...
            const qry = `DELETE FROM role WHERE role_id = ${req.params.id};`;
            //Execute a query
            const results = await db.query(qry, dataQry);
